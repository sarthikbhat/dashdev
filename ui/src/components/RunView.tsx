...
export default function RunView({ runId, workflowName, workflowIcon, onCancel, onBack }: Props) {
  ...
  // Show a starting state while the run data hasn't loaded yet
  if (!run) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16 }}>
        <Spinner />
        <div style={{ fontSize: 14, color: 'var(--dd-text-2)' }}>Starting workflow...</div>
        <div style={{ fontSize: 12, color: 'var(--dd-text-4)' }}>Preparing run environment</div>
      </div>
    );
  }
