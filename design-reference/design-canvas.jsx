...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  // Surface registry for screens that want to programmatically focus a board.
  if (typeof window !== 'undefined') window.__dcRegistry = registry;
  ...
  // Expose a tiny global so screens can navigate between artboards by id.
  // Pass either a slotId "section/board" or just "board" — the first matching
  // section is used.
  React.useEffect(() => {
    window.dcGoTo = (idOrSlot) => {
      if (!idOrSlot) { api.setFocus(null); return; }
      if (idOrSlot.includes('/')) { api.setFocus(idOrSlot); return; }
      // Find first matching slotId by board id (key) across sections.
      const slot = Object.keys(window.__dcRegistry || {}).find(k => k.endsWith('/' + idOrSlot));
      if (slot) api.setFocus(slot);
    };
    return () => { delete window.dcGoTo; };
  }, [api]);
